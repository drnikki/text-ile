...
class Branch {
   ...
   */
  populateBranch({ row = 0, position = 0, out = true, deadZone = null }) {
    // set current node
    const [endPosition, success] = this.addNodeToRow(
      row,
      position,
      out,
      deadZone
   ...
   */
  addNodeToRow(row, position, out, deadZone) {
    // position starts counting at 1
    ...
    if (row === 0) {
      if (!this.rowOpen(1, position + 2) || !this.rowOpen(2, position + 2))
        return fail;
    } else if (row === 1) {
      if (
      ...
      if (
        (this.rowExists(row - 2) &&
          !this.rowOpen(row - 2, position + (out ? 2 : 0))) ||
        (this.rowExists(row + 2) &&
          !this.rowOpen(row + 2, position + (!out ? 2 : 0)))
      )
        return fail;
    ...
    if (deadZone && deadZone.has(row)) {
      ...
      if (deadStart < position + 5 && position < deadEnd) {
        // past start and before end of dead zone
        return fail;
      }
    }
    ...
    if (position > this.rows[row].length) {
      // adding to end of row
      this.rows[row] += " ".repeat(position - this.rows[row].length) + newNode;
    } else {
      //adding to middle of row
      this.rows[row] =
        this.rows[row].slice(0, position) +
        newNode +
        this.rows[row].slice(position + newNode.length);
    }
  ...
  toString() {
    ...
    const lastOddIndex =
      this.rows.length - ((this.rows.length - 1) % 2 === 0 ? 2 : 1);
    ...
    if (!this.directionRight) {
      row =
        "&nbsp;".repeat(Branch.maxWidth - row.length) +
...
const centerPiece = (long = true) => {
  let piece = ("&nbsp;".repeat(6) + "<br/>").repeat(5); // blank space at top
...
const addBough = (background) => {
  ...
  backRows[0] =
    ...
    boughPiece(1) +
    boughPiece(2) +
    boughPiece(3).repeat(2) +
    "/   *";
  backRows[1] =
    "     #/|" +
    boughPiece(1).repeat(2) +
    boughPiece(2) +
    boughPiece(3).repeat(2) +
    "/";
  backRows[2] =
    backRows[2].slice(0, 7) +
    boughPiece(4) +
    boughPiece(1).repeat(2) +
    boughPiece(2) +
    boughPiece(3) +
  ...
  backRows[3] =
    ...
    "/";
  backRows[4] =
    backRows[4].slice(0, 17) +
    boughPiece(4) +
    boughPiece(1).repeat(2) +
    boughPiece(2) +
    "//";
  backRows[5] =
    backRows[5].slice(0, 22) + boughPiece(4) + boughPiece(1).repeat(2) + "\\\\";
  backRows[6] =
    backRows[6].slice(0, 27) + boughPiece(4) + boughPiece(1) + "\\\\";
  ...
  backRows[s + 3] =
    "\\\\|" + boughPiece(3) + "/   *" + backRows[s + 3].slice(13);
  backRows[s + 4] =
    "\\\\|" + boughPiece(3).repeat(2) + "/  &*" + backRows[s + 4].slice(18);
  backRows[s + 5] =
    "//|" +
    boughPiece(2) +
    boughPiece(3).repeat(2) +
    "/~~~*" +
    backRows[s + 5].slice(23);
  backRows[s + 6] =
    " /|" +
    boughPiece(1) +
    boughPiece(2) +
  ...
  if (long) {
    ...
    for (let i = 1; i < 4; i++) {
      ...
      leftBranches[i].populateBranch(
        i === 3 ? { deadZone: deadZones.bottomLeft } : {}
      );
    ...
    if (numRows < 34)
      leftpiece += ("&nbsp;".repeat(16) + "&<br/>").repeat(34 - numRows);
    // -- right
    ...
    let numRows = leftpiece.match(/br/g).length;
    if (numRows < 16)
      leftpiece += ("&nbsp;".repeat(16) + "&<br/>").repeat(16 - numRows);
    // -- right
