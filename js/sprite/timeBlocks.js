...
export default function printTimeBlocks(columnLeft = true) {
  ...
  if (columnLeft) {
    ...
    // the column of numbers is on the right.
  ...
  let regularRows = getRandomInt(
    blockConfig.get("distanceBetweenMin"),
    blockConfig.get("distanceBetweenMax")
  );
  ...
  // how tall is it?
  let blockHeight = getRandomInt(
    blockConfig.get("minHeight"),
    blockConfig.get("maxHeight")
  );
...
function printRegularRows(columnLeft = true) {
  if (columnLeft) {
    ...
    return (
      numToSpace(15) + getTimestamp().toString().split("").join(" ") + "<br />"
    );
  }
}
...
function printtimeRightlocksLeo() {
  ...
  let regularRows =
    Math.floor(
      Math.random() *
        (blockConfig.get("blockSpaceMax") -
          blockConfig.get("blockSpaceMin") +
          1)
    ) + blockConfig.get("blockSpaceMin");
  for (let i = 0; i < regularRows; i++) {
  ...
  let heightOverlap =
    Math.floor(
      Math.random() *
        (blockConfig.get("maxHOverlap") - blockConfig.get("minHOverlap") + 1)
    ) + blockConfig.get("minHOverlap");
  ...
  for (let i = 0; i < heightOverlap; i++) {
    ...
    let blockA = timeLeft
      .toString()
      .substring(0, charOverlap + 1)
      .split("")
      .join(" ");
    let mergedBlock = mergeString(
      timeLeft.substring(charOverlap, 14),
      timeRight.substring(0, charOverlap + 1)
    );
